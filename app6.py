...
if prompt := st.chat_input("복지 서비스에 대해 궁금한 점을 물어보세요."):
    ...
    with st.chat_message("assistant"):
        ...
        4. 문서에 전화번호 뒷자리 네자리만 있을 때는 앞에 "053-667-"를 붙혀서 표시합니다.
        5. **정보 부재 시**: 문서에 내용이 없다면 지어내지 말고 "죄송합니다. 해당 정보가 확인되지 않아 답변을 할 수 없습니다."라고 안내하세요.

        ## 인터넷 주소(URL) 안내 지침
        1. 인터넷 주소를 안내할 때는 반드시 **"사이트명 (URL)"** 형식으로 안내하세요. 
          - 예: 문화누리 홈페이지 (https://www.mnuri.kr)
        2. 주소 앞뒤에는 반드시 **공백**을 한 칸씩 두어 링크가 다른 글자와 섞이지 않게 하세요.
        3. 주소 중간에 공백이 들어가지 않도록 주의하고, 반드시 **https://**를 포함한 전체 주소를 작성하세요.
            ...
            def stream_generator():
                for chunk in response_stream:
                    ...
                    if chunk.text:
                        ...
                        # 2. 프로토콜이 없는 www. 주소 보정
                        text = re.sub(r'(?<!https://)(?<!http://)www\.', r'https://www.', text)
            
                        # 3. ✅ URL을 찾아 앞뒤 공백을 확보하여 가독성과 링크 활성화를 동시에 해결
                        # 이 로직을 통해 주소가 텍스트로 명확히 표시됩니다.
